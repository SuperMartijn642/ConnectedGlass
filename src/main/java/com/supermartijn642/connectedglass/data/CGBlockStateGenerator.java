...
public class CGBlockStateGenerator extends BlockStateGenerator {
    ...
    private void createPaneBlockState(String identifier, Block pane){
        this.blockState(pane)
            ...
            .multipart(condition -> condition.requireProperty(BlockStateProperties.NORTH, true), variant -> variant.model(identifier + "_side_north"))
            .multipart(condition -> condition.requireProperty(BlockStateProperties.EAST, true), variant -> variant.model(identifier + "_side_east", 0, 90))
            .multipart(condition -> condition.requireProperty(BlockStateProperties.SOUTH, true), variant -> variant.model(identifier + "_side_south"))
            .multipart(condition -> condition.requireProperty(BlockStateProperties.WEST, true), variant -> variant.model(identifier + "_side_west", 0, 90))
