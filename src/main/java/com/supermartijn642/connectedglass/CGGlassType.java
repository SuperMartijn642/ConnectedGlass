...
public enum CGGlassType {
    ...
    public String getPaneRegistryName(EnumDyeColor color){
    ...
    public void registerItems(RegistrationHandler.Helper<Item> helper){
        this.blocks.forEach(block -> {
            ...
            helper.register(identifier.getResourcePath(), new BaseBlockItem(block, ItemProperties.create().group(ConnectedGlass.GROUP)));
        });
        this.panes.forEach(pane -> {
            ResourceLocation identifier = Registries.BLOCKS.getIdentifier(pane);
            helper.register(identifier.getResourcePath(), new BaseBlockItem(pane, ItemProperties.create().group(ConnectedGlass.GROUP)));
        });
    }
